    ...
    <div class="container">
        <div class="box form-box">
            ...
            if (isset($_POST['submit'])) {
                ...
                $user_question = $_POST['user_question']; // Retrieve the unique word

                // Ensure the unique word contains no spaces and is only one word
                    ...
                    if ($stmt_email->rowCount() > 0) {
                        ...
                        if ($stmt_connum->rowCount() > 0) {
                            ...
                            if (isset($_FILES['user_pic']) && $_FILES['user_pic']['error'] === UPLOAD_ERR_OK) {
                                ...
                                $file_size = $_FILES['user_pic']['size']; // Add this line to get the file size

                                // Check if the file size exceeds the capacity of MEDIUMBLOB
                                $max_allowed_size = 16 * 1024 * 1024; // 16 MB in bytes
                                ...
                                // Insert data into the database including the image data and hashed unique word
                                $insert_stmt = $conx_customer->prepare("INSERT INTO users (user_fname, user_lname, user_age, user_email, user_pass, user_connum, user_question, user_type, user_regdate, user_pic, user_status) VALUES (:fname, :lname, :age, :email, :pass, :connum, :question, :type, :regdate, :user_pic, 'activated')");
                                ...
                                $insert_stmt->bindParam(':question', $hashed_question); // Bind the hashed unique word
                                $insert_stmt->bindParam(':type', $user_type);
                                $insert_stmt->bindParam(':regdate', $current_datetime);
                                $insert_stmt->bindParam(':user_pic', $user_pic_data, PDO::PARAM_LOB); // use PDO::PARAM_LOB to insert large objects (BLOBs)
                                ...
                                if ($insert_stmt->execute()) {
                                    ...
                                    $insert_audit_stmt = $conx_customer->prepare($insert_audit_query);
                                    $insert_audit_stmt->bindParam(':user_id', $conx_customer->lastInsertId(), PDO::PARAM_INT);
